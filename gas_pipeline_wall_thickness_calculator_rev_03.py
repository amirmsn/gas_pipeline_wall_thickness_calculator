...
def calculate():
    try:
        ...
        error_message = " Invalid input for design pressure!"
        inputerror(error_message)

    try:
        design_temperature = float(entDesignTemperature.get())
    except:
        error_message = " Invalid input for design temperature!"
        inputerror(error_message)
    ...
    try:
        ...
        error_message = " Invalid input for design factor!"
        inputerror(error_message)
    ...
    try:
        ...
        error_message = " Invalid input for corrosion allowance!"
        inputerror(error_message)
    ...
    try:    
        ...
        error_message = " Invalid input for NPS!"
        inputerror(error_message)
    ...
    try:
        ...
        error_message = " Invalid input for pipe garde"
        inputerror(error_message)
    ...
    try:
        ...
        error_message = " Invalid input for pipe spec. №!"
        inputerror(error_message)
    ...
    y_axis_lst_overall = []
    for key, values in w_thk_inch_dic.items():
        y_axis_lst = []
        for i in values:
            y_axis_lst.append(i)
        y_axis_lst_overall.append(y_axis_lst)
    
    x_axis_lst_overall = []
    for i in range(len(y_axis_lst_overall)):
        x_axis_lst = []
        for j in range(len(y_axis_lst_overall[i])):
            x_axis_lst.append(o_d_inch_lst[i])
        x_axis_lst_overall.append(x_axis_lst)
    ...
    # Embeding the plot in the GUI
    figure_frame = plt.figure(figsize=(8, 5))
    
    canvas = FigureCanvasTkAgg(figure_frame, master=main_window)
    
    canvas.draw()
    
    canvas.get_tk_widget().grid(row=0, column=3, rowspan=12, padx=8)
    
    toolbar = NavigationToolbar2Tk(canvas, main_window, pack_toolbar=False)
    toolbar.grid(row=12, column=3, padx=2, sticky="w")

    # Clearing the canvas area for the new plot
    plt.clf()
    
    for i in range(len(y_axis_lst_overall)):
        plt.scatter(x_axis_lst_overall[i], y_axis_lst_overall[i], s=2)
        plt.xticks(o_d_inch_lst, n_p_s_lst, fontsize=8)
        
    plt.yticks(w_thk_inch_lst, fontsize=8)
    plt.vlines(x=o_d_inch, ymin=0, ymax=w_thk_inch_std, colors="gray", linestyles="--", linewidth=0.5)
    plt.hlines(y=w_thk_inch_std, xmin=0, xmax= o_d_inch, colors="red", linestyles="--", linewidth=0.6)
    
    plt.xlim([0, 81])
    plt.ylim([0, 2.6])
    
    plt.xlabel('Nominal Pipe Sizes (NPS)', font="Helvetica 7 bold")
    plt.ylabel('Corresponding Wall Thicknesses (inch) to NPS ' + n_p_s, font="Helvetica 7 bold")
    
    plt.annotate("Design pressure = " + str(design_pressure) + " psig", xy = (26, 2.5), fontsize=6)
    plt.annotate("Design temperature = " + str(design_temperature) + " °C", xy = (26, 2.4), fontsize=6)
    plt.annotate("NPS = " + str(n_p_s), xy = (26, 2.3), fontsize=6)
    plt.annotate("Pipe grade = " + str(pipe_grade), xy = (26, 2.2), fontsize=6)
    plt.annotate("Design factor = " + str(design_factor), xy = (26, 2.1), fontsize=6)
    plt.annotate("Pipe spec. № = " + str(pipe_spec), xy = (26, 2.0), fontsize=6)
    plt.annotate("Corrrosin allowance = " + str(corrosion_allowance) + " mm", xy = (26, 1.9), fontsize=6)
    
    #plt.axis([o_d_inch - 2, o_d_inch + 2, w_thk_inch_std - 0.250, w_thk_inch_std + 0.250])
    
    w_thk_inch_std = "{:.3f}".format(w_thk_inch_std)
    
    plt.annotate("Selected wall thickness = " + w_thk_inch_std + '"', xy = (26, 1.7), font="Helvetica 6 bold")
    
    lblSelectedWallthicknessInch_stringvar.set(w_thk_inch_std)
    
    #plt.show()
    
    #plt.scatter(x_axis_lst_overall[n_p_s_index], y_axis_lst_overall[n_p_s_index])
    #plt.annotate("---> ANS", (o_d_inch, w_thk_inch_lst[index_of_answer - 1]))
...
dropDesignFactor.config(width=15)
...
dropPipeSpec.config(width=15)

lblCorrosionAllowance = Label(main_window, text="Corrosion allowance: ")
entCorrosionAllowance = Entry(main_window, width=25)
lblCorrosionAllowanceUnit = Label(main_window, text=" mm")

btnCalculate = Button(main_window, text="CALCULATE", width=20, bg="orange", fg="white", command=calculate)
...
lblDisclaimer = Label(main_window, text="This application is dedicated to the Pipeline Department of \nSazeh " 
                      "Consultants, Engineering and Construction Co. \nIt is made as per ASME B31.8 - 2014, API Specification 5L," 
                      "\n45th edition, and ASME B36.10M - 2004 requirements."
                      "\nIt is still under development. In case of any breakdown or error, \ndon't hesitate to contact the developer via amirmsn@gmail.com.", justify=LEFT, font="Helvetica 8")

# <<==== Shoving the widgets into the window
lblDesignPressure.grid(row=0, column=0, sticky="w", padx=2)
entDesignPressure.grid(row=0, column=1, ipady=5, pady=5, sticky="w", padx=2)
lblDesignPressureUnit.grid(row=0, column=2, sticky="w")

lblDesignTemperature.grid(row=1, column=0, sticky="w", padx=2)
entDesignTemperature.grid(row=1, column=1, ipady=5, pady=5, sticky="w", padx=2)
lblDesignTemperatureUnit.grid(row=1, column=2, sticky="w")

lblNPS.grid(row=2, column=0, sticky="w", padx=2)
dropNPS.grid(row=2, column=1, ipady=5, pady=5, sticky="w", padx=2, columnspan=2)

lblPipeGrade.grid(row=3, column=0, sticky="w", padx=2)
dropPipeGrade.grid(row=3, column=1, ipady=5, pady=5, sticky="w", padx=2, columnspan=2)
#lblPipeGradeUnit.grid(row=3, column=2, sticky="w")

lblDesignFactor.grid(row=4, column=0, sticky="w", padx=2)
dropDesignFactor.grid(row=4, column=1, ipady=5, pady=5, sticky="w", padx=2, columnspan=2)

lblPipeSpec.grid(row=5, column=0, sticky="w", padx=2)
dropPipeSpec.grid(row=5, column=1, ipady=5, pady=5, sticky="w", padx=2, columnspan=2)

lblCorrosionAllowance.grid(row=6, column=0, sticky="w", padx=2)
entCorrosionAllowance.grid(row=6, column=1, ipady=5, pady=10, sticky="w", padx=2)
...
lblCalculatedWallThickness.grid(row=9, column=0, sticky="w", padx=2)
lblCalculatedWallThicknessValue.grid(row=9, column=1, pady=5, sticky="e")
lblCalculatedWallThicknessUnit.grid(row=9, column=2, sticky="w")

lblSelectedWallthicknessInch.grid(row=10, column=0, sticky="w", padx=2, columnspan=2)
lblSelectedWallthicknessInchValue.grid(row=10, column=1, pady=5, sticky="e")
lblSelectedWallthicknessInchUnit.grid(row=10, column=2, sticky="w")

separator_02.grid(row=11, columnspan=3, pady=10, sticky="ew")

lblDisclaimer.grid(row=12, columnspan=3, sticky="w", padx=2)


# Preventing the size of the main window to be changed by the user
main_window.resizable(False, False)

#main_window.geometry("580x550")
